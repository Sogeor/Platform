...
{
    __asm__ volatile("call __boot_dap_process" : : "a"(sectors), "b"(buf_off), "c"(buf_seg), "d"(lba_low));
}
...
{
    __asm__ volatile("call __boot_print" : : "S"(s));
}

__attribute__((unused, section(".main")))
extern void __main_println(const char *s)
{
    __asm__ volatile("call __boot_println" : : "S"(s));
}